...
describe('Form', () => {
    ...
    it('Create local state', () => {
        ...
        expect(formInstance.state).toMatchObject(defaultInputData, { defaultFormInputs: formInputs });
    });

    it('Set input data', () => {
        const formInstance = createInstanceWithState()
        formInstance.setInputData(dataToUpdate, false);
        expect(formInstance.state).toMatchObject(inputDataAfterUpdate, { defaultFormInputs: formInputs });
    });
